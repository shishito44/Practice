...
string? menutestext = null;

using (StreamReader sr = new StreamReader(meetingplompt))
{
    meetingtext = sr.ReadToEnd();
...
using (StreamReader sr = new StreamReader(minutesplompt))
{
    menutestext = sr.ReadToEnd();
    Console.WriteLine(menutestext);
}
